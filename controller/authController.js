...
const login = async (req, res, next) => {
  try {
    ...
    if (!user.verify) return next(new ErrorHandler(404, "User Not Found"));
  ...
  try {
    ...
    if (oldotp) {
      ...
      if (dateNow < otpDate + 10)
        return next(
          new ErrorHandler(400, "Wait for 10 seconds to resend mail.")
        );
...
const signup = async (req, res, next) => {
  try {
    ...
    user.name = name;
    (user.student_no = student_no), (user.year = year);
    user.course = course;
    user.branch = branch;
    user.POR = POR;
    ...
    return res
      ...
      .json({
        success: true,
        msg: `Welcome to Samriddhi Prawah, ${name}!`,
        user,
        refreshToken,
        accessToken,
      });
...
const ChangePassword = async (req, res, next) => {
  try {
    ...
    return res
      ...
      .json({
        success: true,
        msg: `Welcome to Samriddhi Prawah, ${user.name}!`,
        user,
        refreshToken,
        accessToken,
      });
